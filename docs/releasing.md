...
当前是测试版本准备，不是正式发布公告。保留 package.json、Tauri、Cargo、Python 的 0.1.0，Cargo.lock 同步检查；不自动创建 Git 标签或 GitHub Release。
...
python scripts/prepare_release.py --artifact src-tauri/target/release/bundle/nsis/WiseTodo_0.1.0_x64-setup.exe
```
...
- [ ] 核对 Skill/Tool 初次安装说明，确保不会误导用户以为示例工具已自动启用。
...
Windows x64：`src-tauri/target/release/bundle/nsis/WiseTodo_0.1.0_x64-setup.exe`。构建详情见 [building.md](building.md)。目录已被 Git 忽略；无需将二进制或校验文件提交到源码仓库。本地准备完成不代表上述人工关卡通过。
