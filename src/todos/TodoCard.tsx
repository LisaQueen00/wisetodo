...
export function TodoCard({ todo, expanded, onToggle, actions, editing, editLocked, onEdit, reorder }: {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="min-w-0 flex-1 font-medium">
          <button
            ...
            className="flex w-full cursor-pointer items-start gap-2 rounded-md text-left hover:text-[var(--theme-text-primary)]"
          >
            <ChevronRight
              aria-hidden="true"
              focusable="false"
              className={`mt-1 size-4 shrink-0 text-[var(--theme-text-secondary)] transition-transform motion-reduce:transition-none ${expanded ? "rotate-90" : ""}`}
            />
      ...
      {actions && (
        <div className="mt-3 flex gap-3 text-xs text-[var(--theme-text-secondary)]">
      ...
      {savingItem && <p role="status" className="mt-2 text-xs text-[var(--theme-text-secondary)]">正在保存完成状态…</p>}
      {completionError && <p role="alert" className="mt-2 text-xs text-[var(--theme-status-error)]">完成状态保存失败，未更改原状态，请重新勾选重试。</p>}
      <div id={panelId} hidden={!expanded}>
        {expanded && (
          <ol
            aria-label={`${todo.topic}的子项`}
            className="mt-4 list-decimal space-y-2 border-t border-[var(--theme-border-normal)] pt-4 pl-6 text-sm text-[var(--theme-text-secondary)] marker:text-[var(--theme-text-muted)]"
          >
            {todo.items.map((item) => (
              <li key={item.id} data-completed={item.completed} className="pl-1 whitespace-pre-wrap break-words [overflow-wrap:anywhere]">
                <div className="flex items-start justify-between gap-3">
                  <span data-completed={item.completed} className={`min-w-0 flex-1 ${item.completed ? "text-[var(--todo-completed)] line-through decoration-current/40" : ""}`}>{item.topic}</span>
                  <input
                    type="checkbox"
