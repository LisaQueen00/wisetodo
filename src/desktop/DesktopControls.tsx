...
export function DesktopControls({ api }: { api: DesktopApi }) {
  ...
  return <div className="max-w-64 text-xs text-white/65">
    <div className="flex items-center gap-3">
      <button aria-label="窗口置顶" aria-pressed={state?.pinned ?? false} disabled={!state || busy}
        className="flex items-center gap-1 rounded bg-white/10 px-2 py-1 disabled:opacity-40"
    ...
    {state?.abnormal_exit && <div role="alert" className="mt-2 rounded border border-amber-300/40 p-2">
