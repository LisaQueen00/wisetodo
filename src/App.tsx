...
function App({ loadTodos, mutations, preview = false, sessionApi, settingsApi, desktopApi }: {
  ...
  return (
    <main style={{ "--panel-alpha": opacity / 100, "--surface-alpha": (opacity - 60) / 40 } as CSSProperties} className="app-panel flex h-dvh min-h-[520px] flex-col overflow-hidden bg-[var(--surface-window)] text-[var(--text-primary)]">
      <ErrorNotice />
        <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[var(--theme-border-normal)] p-3">
          <div>
            <h1 className="text-xl font-semibold tracking-tight">WiseTodo</h1>
            <p className="mt-1 text-xs text-[var(--theme-text-muted)]">一件一件，慢慢完成。</p>
          </div>
          <button ref={chatButton} aria-expanded={chatOpen} aria-controls="chat-overlay"
            className="rounded-lg bg-[var(--theme-chat-message-background)] px-3 py-2 text-sm" onClick={() => setChatOpen((value) => !value)}>
            {chatOpen ? "收起 Chat" : "打开 Chat"}{chatRunning && <span role="status" className="ml-2 text-xs">执行中</span>}
          </button>
          <ThemeSettings controller={theme} />
          <details className="w-full text-xs text-[var(--theme-text-secondary)]"><summary className="cursor-pointer">桌面选项</summary>
            ...
            <button className="mt-2 rounded bg-[var(--theme-chat-message-background)] px-2 py-1" onClick={() => changeOpacity(100)}>恢复不透明背景</button>
      ...
      <section aria-label="Todo 工作区" inert={chatOpen} aria-hidden={chatOpen}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3" tabIndex={0} aria-label="Todo 列表滚动区">
          {preview && <p className="mb-4 rounded-lg bg-[var(--theme-chat-message-background)] p-3 text-xs text-[var(--theme-text-secondary)]">示例数据预览 · 不会写入数据库</p>}
          {loadTodos ? <TodoWorkspace key={preview ? "preview" : "live"} loadTodos={loadTodos} mutations={mutations} refreshToken={todoRevision} /> : (
            <div role="status" className="rounded-2xl border border-dashed border-[var(--theme-border-normal)] p-6">
              <p>请通过桌面应用查看 Todo</p>
              <p className="mt-2 text-sm text-[var(--theme-text-secondary)]">浏览器无法连接本地数据库，请启动 WiseTodo 桌面应用。</p>
            </div>
          )}
        </div>
      </section>
      ...
      <aside id="chat-overlay" aria-label="Chat 工作区" inert={!chatOpen} aria-hidden={!chatOpen}
        ...
        {settingsApi ? <SettingsPanel api={settingsApi} /> : <h2 className="mb-5 shrink-0 text-sm font-medium text-[var(--theme-text-secondary)]">Chat</h2>}
        {sessionApi ? <SessionPanel api={sessionApi} visible={chatOpen} onRunningChange={setChatRunning} loadTodos={loadTodos} onCommitted={onCommitted} /> : <div className="min-h-0 flex-1 overflow-y-auto rounded-2xl border border-dashed border-[var(--theme-border-normal)] p-6 text-sm leading-7 text-[var(--theme-text-secondary)]">
          从一个想做的事情开始。<br />对话功能接入后，可以在这里讨论并创建任务。
        </div>}
        {!sessionApi && <textarea disabled aria-label="聊天输入（待接入）" placeholder="聊天功能待接入" rows={3}
          className="mt-4 w-full shrink-0 resize-none rounded-xl border border-[var(--theme-border-normal)] bg-[var(--theme-chat-message-background)] p-3 text-sm placeholder:text-[var(--theme-text-muted)]" />}
      </aside>
      </div>
    </main>
  );
