...
#[tauri::command]
async fn settings_test(backend: tauri::State<'_, Arc<sidecar::TodoBackend>>) -> Result<serde_json::Value, String> {
    let backend = Arc::clone(backend.inner());
    tauri::async_runtime::spawn_blocking(move || backend.settings_test())
